...
import { menuItems as staticMenuItems } from "@/components/Menu";
...
const Orders = () => {
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      // Merge static menu items with Supabase items, prefer Supabase price if available
      let mergedMenuItems = [...staticMenuItems];

      if (data) {
        // Add Supabase items that are not in staticMenuItems by name
        data.forEach(dbItem => {
          if (!mergedMenuItems.some(staticItem => staticItem.name === dbItem.name)) {
            mergedMenuItems.push(dbItem);
          }
        });
      }

      // Remove duplicates by name, prefer Supabase price if available
      const uniqueMenuItems = mergedMenuItems.reduce((acc, item) => {
              ...
              <div className="space-y-2">
                ...
                {selectedItems.map((item, index) => {
                  ...
                  const filteredMenuItems = menuItems
                    ...
                    // Ensure only unique names are shown in the dropdown
                    .filter((menuItem, idx, arr) =>
                      arr.findIndex(m => m.name === menuItem.name) === idx
                    );

                  // If search is empty, show all items
                  const allMenuItems = item.searchQuery
                    ? filteredMenuItems
                    : menuItems;
                          ...
                          <CommandList>
                            ...
                            <CommandGroup>
                              {allMenuItems.map(menuItem => (
                                <CommandItem
                                  key={menuItem.name}
                                  onSelect={() => updateSelectedItem(index, 'menuItemId', menuItem.id ?? menuItem.name)}
                                >
                                  {menuItem.name} - ₹{menuItem.fullPrice ?? menuItem.price ?? menuItem.halfPrice}
                                </CommandItem>
                              ))}
                            </CommandGroup>
                          </CommandList>
